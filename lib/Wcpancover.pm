...
has config_file => sub {
  my $self = shift;
  return $ENV{WCPANCOVER_CONFIG} if $ENV{WCPANCOVER_CONFIG};

  return rel2abs('wcpancover.conf', $self->home_path);
};
...
sub startup {
  ...
  $app->plugin(
    Config => {
      ...
      default => {
        'db_connect' => [
          'dbi:SQLite:dbname=' . $app->home->rel_file('wcpancover.db'),
          undef,
          undef,
          {'sqlite_unicode' => 1}
        ],
...

  {
    ...
    $app->static->paths->[0] = -d $public ? $public : catdir(dist_dir('WCpanCover'), 'files','public');
    my $static_path = $app->static->paths->[0];
    #print STDERR '$static_path: ',$static_path,"\n";

    my $templates = catdir($lib_base, 'templates');
    $app->renderer->paths->[0] = -d $templates ? $templates : catdir(dist_dir('WCpanCover'), 'files', 'templates');
  }

  push @{$app->commands->namespaces}, 'WCpanCover::Command';
...
L<WCpanCover> is a Perl web apllication.

=head1 INSTALLATION

L<WCpanCover> uses well-tested and widely-used CPAN modules, so installation should be as simple as
...
Although most of L<WCpanCover> is controlled by a configuration file, a few properties must be set before that file can be read. These properties are controlled by the following environment variables.

=over

=item C<WCPANCOVER_HOME>

This is the directory where L<WCpanCover> expects additional files. These include the configuration file and log files. The default value is the current working directory (C<cwd>).

=item C<WCPANCOVER_CONFIG>

This is the full path to a configuration file. The default is a file named F<aprojo.conf> in the C<WCPANCOVER_HOME> path, however this file need not actually exist, defaults may be used instead. This file need not be written by hand, it may be generated by the C<aprojo config> command.

=back

=head2 The F<aprojo> command line application

L<WCpanCover> installs a command line application, C<aprojo>. It inherits from the L<mojo> command, but it provides extra functions specifically for use with AproJo.
...
L<WCpanCover> does not need to be configured, however it is recommended to do so to set your application's secret.
...
This step is required. Run C<wcpancover setup> to setup a database. It will use the default DBI settings (SQLite) or whatever is setup in the C<WCPANCOVER_CONFIG> configuration file.

=head1 RUNNING THE APPLICATION

 $ wcpancover daemon

After the database is has been setup, you can run C<wcpancover daemon> to start the server.

You may also use L<morbo> (Mojolicious' development server) or L<hypnotoad> (Mojolicious' production server). You may even use any other server that Mojolicious supports, however for full functionality it must support websockets. When doing so you will need to know the full path to the C<wcpancover> application. A useful recipe might be
...
Logging in L<WCpanCover> is the same as in L<Mojolicious|Mojolicious::Lite/Logging>. Messages will be printed to C<STDERR> unless a directory named F<log> exists in the C<WCPANCOVER_HOME> path, in which case messages will be logged to a file in that directory.
...
L<http://github.com/wollmers/WCpanCover>
